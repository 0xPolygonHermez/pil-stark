...
module.exports.proof2zkin = function proof2zkin(p) {
    ...
    for (let i=0; i<friProof[0].polQueries.length; i++) {
        ...
        if (friProof[0].polQueries[0][1][0].length) {
            zkin.s0_vals3[i] = friProof[0].polQueries[i][1][0];
            zkin.s0_siblings3[i] = friProof[0].polQueries[i][1][1];
        }
       
        zkin.s0_vals4[i] = friProof[0].polQueries[i][2][0];
        zkin.s0_siblings4[i] = friProof[0].polQueries[i][2][1];

        zkin.s0_valsC[i] = friProof[0].polQueries[i][3][0];
        zkin.s0_siblingsC[i] = friProof[0].polQueries[i][3][1];
    }
