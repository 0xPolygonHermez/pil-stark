...
async function linearHash(wasmModule, buffIn, width, st_i, st_n, arity) {
    ...
    const pIn = alloc(wasmMem, arity*32);
    const pSt = alloc(wasmMem, 32);

    const in64 =new BigUint64Array(wasmMem.buffer, pIn, 4*arity);
    ...
    function hash(vals) {
        ...
        for(let i=0; i<vals.length; i++) {
            ...
            p++;
            if (p==arity*4) {
                instance.exports.poseidon(pSt, pIn, arity, pSt, 1);
                p=0;
            }
            if (i%3 == 2) {
                in64[p] = 0n;
...
async function merkelizeLevel(wasmModule, buffIn, st_i, st_n, arity) {
    ...
    for (let i=0; i<nOps; i++) {
        ...
        const sBuff = new BigUint64Array(buffIn.buffer, buffIn.byteOffset + i*(arity*32), arity*4);
        in64.set(sBuff);
        instance.exports.poseidon(pSt, pIn, arity, pOut, 1);
