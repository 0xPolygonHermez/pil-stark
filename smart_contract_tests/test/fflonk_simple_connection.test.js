...
describe("Fflonk connection sm", async function () {
    ...
    it("It should create the pols main", async () => {
        ...
        const zkeyFilename =  path.join(__dirname, "../../", "tmp", "fflonk_simple_connection.zkey");
        const constExtFilename =  path.join(__dirname, "../../", "tmp", "fflonk_simple_connection.ext.const");
        ...
        fs.writeFileSync("./tmp/contracts/shplonk_verifier_simple_connection.sol", verifierCode.verifierShPlonkCode, "utf-8");

        await run("compile");

        const ShPlonkVerifier = await ethers.getContractFactory("./tmp/contracts/shplonk_verifier_simple_connection.sol:ShPlonkVerifier");
        const shPlonkVerifier = await ShPlonkVerifier.deploy();

        let shPlonkAddress = (await shPlonkVerifier.deployed()).address;

        const PilFflonkVerifier = await ethers.getContractFactory("./tmp/contracts/pilfflonk_verifier_simple_connection.sol:PilFflonkVerifier");
