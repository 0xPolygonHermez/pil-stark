...
describe("Verify Evals Goldilocks Circuit Test", function () {
    ...
    it("Should check that verify eval calculates proper output if all inputs are set to max value", async () => {
        ...
        let circuitVerifyEvals = await wasm_tester(path.join(__dirname, "circom", "verifyEvals.bn128.test.circom"), {O:1, include: ["circuits.bn128", "node_modules/circomlib/circuits"]});

        const w1 = await circuitVerifyEvals.calculateWitness({});
    ...
    it("Should check that verify eval calculates proper output if all inputs are set to max value and all subs are zero", async () => {
        ...
        let circuitVerifyEvalsMin = await wasm_tester(path.join(__dirname, "circom", "verifyEvalsMin.bn128.test.circom"), {O:1, include: ["circuits.bn128", "node_modules/circomlib/circuits"]});

        const w2 = await circuitVerifyEvalsMin.calculateWitness({});
