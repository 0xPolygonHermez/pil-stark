...
describe("Linear Hash Circuit Test", function () {
    ...
    before( async() => {
        circuitBig = await wasm_tester(path.join(__dirname, "circom", "linearhashbig.gl.test.circom"), {O:1, prime: "goldilocks", verbose:true});
        circuitSmall = await wasm_tester(path.join(__dirname, "circom", "linearhashsmall.gl.test.circom"), {O:1, prime: "goldilocks", verbose:true});
    });
